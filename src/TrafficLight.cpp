...
T MessageQueue<T>::receive()
{
    ...
    _queue.clear(); // using pop_back, vehicles do no stop
...
void TrafficLight::waitForGreen()
{
    ...
    while(true){
        TrafficLightPhase recMsg = _trafficLightQueue.receive();
        if (recMsg)
            break;
    }
...
void TrafficLight::cycleThroughPhases() {
    ...
    // random generator for tooglinf time
    std::random_device rd;
    std::mt19937 mt(rd());
    std::uniform_real_distribution<double> generator(4.0, 6.0);
    double loopTime = generator(mt);
    ...
    while (true) {
        ...
        // elapsed comes in miliseconds, our random generator for loopTime works in double type and seconds
        if (double(elapsed)/1000 > loopTime){
            // when time is due change phase and restart timers and send message
            start = std::chrono::high_resolution_clock::now();
            loopTime = generator(mt);
